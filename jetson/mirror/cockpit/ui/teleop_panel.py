"""Dead-man /cmd_vel teleop panel for onboard cockpit."""
...
class TeleopPanel(QWidget):
    ...
    def __init__(
        ...
        self._web_control_url = (web_control_url or "").strip()
    ...
    def _build_ui(self) -> None:
        ...
        # 右上角独立按钮：不受 ROS2 就绪状态禁用
        if self._web_control_url:
            web_btn = QPushButton("Web控制")
            web_btn.setMinimumHeight(42)
            web_btn.setToolTip(self._web_control_url)
            web_btn.clicked.connect(self._open_web_control)
            grid.addWidget(web_btn, 0, 2)
    ...
    def _open_web_control(self) -> None:
        from ui.web_control_window import open_or_raise_web_control

        self._web_window = open_or_raise_web_control(
            self.window(), self._web_control_url, self._web_window
        )
